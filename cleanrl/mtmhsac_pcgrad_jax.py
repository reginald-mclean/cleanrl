...
from metaworld.env_dict import ALL_V2_ENVIRONMENTS, MT10_V2
...
def parse_args():
    ...
    parser.add_argument("--learning-starts", type=int, default=5e2, help="timestep to start learning")
...
class Agent:
    ...
    def get_action_train(
        self, obs: np.ndarray, key: jax.Array
...
def update(
    ...
    Tuple[TrainState, CriticTrainState, TrainState], dict, jax.Array
]:
...
def critic_loss(critic, params: flax.core.FrozenDict, alpha_val: jax.Array, q_values, next_action_log_probs, batch: NamedTuple, gamma):
    min_qf_next_target = jnp.min(q_values, axis=0).reshape(
        -1, 1
    ) - alpha_val * next_action_log_probs.sum(-1).reshape(-1, 1)
    next_q_value = jax.lax.stop_gradient(
...
@partial(jax.jit)
def actor_loss(params: flax.core.FrozenDict, actor_loss_key, batch, actor, critic, alpha_val):
    action_samples, log_probs, _ = sample_and_log_prob(
        actor, params, batch.observations, batch.task_ids, actor_loss_key
    )
    q_values = critic.apply_fn(
        critic.params, batch.observations, action_samples, batch.task_ids
    )
    min_qf_values = jnp.min(q_values, axis=0)
    policy_loss = (alpha_val * log_probs.sum(-1).reshape(-1, 1) - min_qf_values).mean()
    return policy_loss
...
def update(
    ...
    # Apply weight decay
    actor = actor.replace(params=jax.tree_util.map(lambda x: x * (1 - weight_decay), actor.params))
...
def split_batch_by_task(batch: Batch, num_tasks: int = 10):
    # Extract task IDs (assuming they're one-hot encoded in the last 10 elements)
    task_ids = np.argmax(batch.observations[:, -num_tasks:], axis=1)

    # Initialize lists to hold data for each task
    task_data = [{
        'observations': [],
        'actions': [],
        'rewards': [],
    ...
    for i, task_id in enumerate(task_ids):
        task_data[task_id]['observations'].append(batch.observations[i])
        task_data[task_id]['actions'].append(batch.actions[i])
        task_data[task_id]['rewards'].append(batch.rewards[i])
        task_data[task_id]['next_observations'].append(batch.next_observations[i])
        task_data[task_id]['dones'].append(batch.dones[i])
        task_data[task_id]['task_ids'].append(task_id)

    # Convert lists to numpy arrays and create Batch objects
    task_batches = []
    for data in task_data:
        if data['observations']:  # Check if there's any data for this task
            task_batches.append(Batch(
                ...
                task_ids=np.array(data['task_ids'])
            ))
        else:
            # Create an empty batch for tasks with no data
            task_batches.append(Batch(
                observations=np.empty((0,) + batch.observations.shape[1:]),
                actions=np.empty((0,) + batch.actions.shape[1:]),
                rewards=np.empty((0,) + batch.rewards.shape[1:]),
                next_observations=np.empty((0,) + batch.next_observations.shape[1:]),
                dones=np.empty((0,) + batch.dones.shape[1:]),
                task_ids=np.empty((0,) + batch.task_ids.shape[1:])
            ))

    return task_batches

def flatten_dict_completely(d):
    leaves, treedef = jax.tree_util.tree_flatten(d)
    flat_array = jnp.concatenate([leaf.ravel() for leaf in leaves])
    shapes = [leaf.shape for leaf in leaves]
    return flat_array, (treedef, shapes)

def unflatten_dict(flat_array, structure):
...
def project_gradients(task_gradients, key):
    num_tasks = len(task_gradients)

    # Flatten gradients and store structures
    flat_grads = []
    structures = []
    for grad in task_gradients:
        flat_grad, structure = flatten_dict_completely(grad)
        flat_grads.append(flat_grad)
        structures.append(structure)

    flat_grads = jnp.array(flat_grads)

    def task_projection(i, args):
        g_pc, key = args
        key, subkey = jax.random.split(key)

        j = jax.random.choice(subkey, num_tasks - 1, shape=())
        j = jnp.where(j < i, j, j + 1)

        gi_flat, gj_flat = g_pc[i], flat_grads[j]

        # Compute cosine similarity
        cos_sim = jnp.dot(gi_flat, gj_flat) / (jnp.linalg.norm(gi_flat) * jnp.linalg.norm(gj_flat))

        def project(gi):
            projection = jnp.dot(gi, gj_flat) / jnp.dot(gj_flat, gj_flat)
            return gi - projection * gj_flat

        gi_new = jax.lax.cond(cos_sim < 0, project, lambda x: x, gi_flat)
        g_pc = g_pc.at[i].set(gi_new)
        return g_pc, key

    g_pc = flat_grads.copy()
    g_pc, _ = jax.lax.fori_loop(0, num_tasks, task_projection, (g_pc, key))

    # Unflatten and sum the projected gradients
    unflattened_grads = [unflatten_dict(g, structures[i]) for i, g in enumerate(g_pc)]
    return jax.tree_util.tree_map(lambda *args: jnp.sum(jnp.stack(args), axis=0), *unflattened_grads)
...
if __name__ == "__main__":
    ...
    value_and_grad_crit = jax.value_and_grad(critic_loss, argnums=1)
    value_and_grad_actor = jax.value_and_grad(actor_loss, argnums=0)
    ...
    for global_step in range(args.total_timesteps // NUM_TASKS):
        ...
        if global_step > args.learning_starts and global_episodic_return:
            ...
            for i in range(args.gradient_steps):
                actor_grads = []
                critic_grads = []
                original_data = rb.sample(args.batch_size)
                data = split_batch_by_task(original_data, num_tasks=NUM_TASKS)

                original_obs, original_tid = split_obs_task_id(original_data.observations, NUM_TASKS)

                action_samples, log_probs, key = sample_and_log_prob(
                    agent.actor, agent.actor.params, original_obs, original_tid, key
                )

                for b in data:
                    observations, task_ids = split_obs_task_id(b.observations, NUM_TASKS)
                    next_observations, _ = split_obs_task_id(b.next_observations, NUM_TASKS)
                    ...
                    next_actions, next_action_log_probs, key = sample_and_log_prob(
                        agent.actor, agent.actor.params, batch.next_observations, batch.task_ids, key
                    )
                    q_values = agent.critic.apply_fn(
                        agent.critic.target_params, batch.next_observations, next_actions, batch.task_ids
                    )

                    key, actor_loss_key = jax.random.split(key)

                    crit_loss, crit_grads = value_and_grad_crit(agent.critic, agent.critic.params, agent.alpha_train_state.params, q_values, next_action_log_probs, batch, args.gamma)
                    act_loss, act_grads = value_and_grad_actor(agent.actor.params, actor_loss_key, batch, agent.actor, agent.critic, agent.alpha_train_state.params)
                    critic_grads.append(crit_grads)
                    actor_grads.append(act_grads)

                '''pc_actor_grads = actor_grads.copy()
                for i in range(NUM_TASKS):
                    grad_1, grad_2 = [], []
                    j = np.random.randint(low=0, high=NUM_TASKS+1)
                    if i != j:
                        for key in actor_grads[0]['params']:
                            for subkey in actor_grads[0]['params'][key]:
                                if len(actor_grads[j]['params'][key][subkey].shape) == 1:
                                    grad_1.extend(actor_grads[j]['params'][key][subkey].tolist())
                                    grad_2.extend(pc_actor_grads[i]['params'][key][subkey].tolist())
                                else:
                                    for h in range(actor_grads[j]['params'][key][subkey].shape[0]):
                                        grad_1.extend(actor_grads[j]['params'][key][subkey][h].tolist())
                                        grad_2.extend(pc_actor_grads[i]['params'][key][subkey][h].tolist())
                                print(actor_grads[j]['params'][key][subkey].shape)
                        grad_1 = np.array(grad_1)
                        grad_2 = np.array(grad_2)
                        print(np.dot(grad_1, grad_2)/(np.linalg.norm(grad_1) * np.linalg.norm(grad_2)))'''
                key, subkey = jax.random.split(key)
                actor_projected = project_gradients(actor_grads, subkey)
                key, subkey = jax.random.split(key)
                critic_projected = project_gradients(critic_grads, subkey)


                def alpha_loss(params: jax.Array):
                    _, task_ids = split_obs_task_id(original_data.observations, NUM_TASKS)
                    log_alpha = task_ids @ params.reshape(-1, 1)
                    return (-log_alpha * (log_probs.sum(-1).reshape(-1, 1) + agent.target_entropy)).mean()
                ...
                agent.alpha_train_state, alpha_vals, alpha_logs = update_alpha(agent.alpha_train_state)
                agent.actor = agent.actor.apply_gradients(grads=actor_projected)
                agent.critic = agent.critic.apply_gradients(grads=critic_projected)
