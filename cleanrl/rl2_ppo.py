    ...
    for update in range(1, num_updates + 1):
        ...
        if len(meta_episode_batch) == 8:
            ...
            for sample in minibatches:
                ...
                import ipdb; ipdb.set_trace()
                agent_dict = agent.get_action_and_value(obs_batch, states_actor_batch, states_critic_batch)
                logprob = agent_dict["logprob"]
                logratio = logprob - old_logprobs_batch
                ratio = logratio.exp()

                action = agent_dict["action"].squeeze(1)

                value = agent_dict["value"].squeeze(1)
                actor_state = agent_dict["actor_state"].squeeze(0)
                critic_state = agent_dict["critic_state"].squeeze(0)
