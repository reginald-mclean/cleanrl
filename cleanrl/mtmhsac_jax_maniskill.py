...
class Maniskill2_Benchmark(Benchmark):
    def __init__(self):
        super().__init__()
        tasks = ['PushChair-v1', 'OpenCabinetDoor-v1', 'OpenCabinetDrawer-v1' 'TurnFaucet-v0', 'PickCube-v0', 'LiftCube-v0']
        self._train_classes = {name : gym.make(name, obs_mode='state', reward_mode='dense') for name in tasks} # obs_space='original'
...
if __name__ == "__main__":
    ...
    envs = make_envs(
        benchmark, args.seed, args.max_episode_steps, use_one_hot=use_one_hot_wrapper, reward_func_version=args.reward_version, normalize_rewards=args.normalize_rewards
    )
