...
def load_roboclip_model(checkpoint, word2vec_path):
    model = s3dg.S3D(
        512, space_to_depth=True, word2vec_path=word2vec_path
    )
    checkpoint = torch.load(checkpoint)
    checkpoint_params = {k.removeprefix("module.").removeprefix("module."): v for k, v in checkpoint["state_dict"].items()}
...
def parse_args():
    ...
    # RoboCLIP
    parser.add_argument('--roboclip-ckpt', type=str, default=None, help='Path to roboclip checkpoint.')
...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"{args.env_id}_{args.exp_name}_rbfix_roboclip"
    ...
    if args.vlm_reward_weight != 0:
        reward_model = load_roboclip_model(args.roboclip_ckpt, WORD2VEC_PATH)
        reward_model = RewardCalculator(reward_model)
        pairs_text = torch.zeros([1, 20]).int().to('cuda:0')
        video = torch.zeros([1, 3, 32, 224, 224]).float().to('cuda:0')
        a, b = reward_model.model(video, pairs_text)

        frames = torch.zeros((args.num_envs, args.max_episode_steps, 3, 224, 224))

        with open(f'{ROBOCLIP_CAPTION_PATH}/raw-roboclip-captions.pkl', 'rb') as f:
        ...
        pairs_text = reward_model.dataloader._get_text(video_id=0, caption=task_desc)
        pairs_text = torch.from_numpy(np.asarray(pairs_text)).to('cuda:0')
        pairs_text = pairs_text.unsqueeze(0)
    ...
    for global_step in range(args.total_timesteps // NUM_TASKS):
        ...
        if args.vlm_reward_weight != 0:
            ...
            batches = torch.zeros((args.num_envs, reward_model.dataloader.max_frames, 3, 224, 224))
            ...
            if current_t >= offset_timestep:
                if current_t >= reward_model.dataloader.max_frames or args.stretch_partial_videos:
                    images = np.linspace(0, current_t, reward_model.dataloader.max_frames, dtype=int)
                else:
